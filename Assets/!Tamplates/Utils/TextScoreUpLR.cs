...
public class TextScoreUpLR : MonoBehaviour
{
    ...
    private void PlayAnimation()
    {
        ...
        if (isUI) {
            ...
            tweener = DOVirtual.Vector3(rectTransform.position,
            rectTransform.position + (Vector3.up * _moveDistance * koef) + (moveOffset * speedOffset * koef),
            _moveDuration,
            (pos) => rectTransform.position = pos)
            .SetEase(_moveEase);
        }
        else
            tweener = DOVirtual.Vector3(transform.position,
            transform.position + (Vector3.up * _moveDistance * koef) + (moveOffset * speedOffset * koef),
            _moveDuration,
